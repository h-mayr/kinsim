...
//
// Updated to v4 to include beam spot size effects
//
...
void kinsim4( int Zb, int Zt, double Ab, double At, double thick /* mg/cm^2 */, double Eb /* MeV/u */,
    double dEb = 0.1 /* MeV/u */, double Ex = 1.0 /* MeV */, double res = 0.6 /* % */,
	double beam_sigma = 1.5 /* mm */, double cd_dist = 28.0 /* mm */, bool flat = false /* angular distribution? */,
	...
	outname += convertFloat(dEb,3) + "MeVu_res" + convertFloat(res,1) + "percent_";
	outname += convertFloat(beam_sigma,3) + "mm.root";
	...
	double com, b_lab, b_en, t_lab, t_en, depth, Eb_real, t_xpos, b_xpos, __xpos;
	TRandom3 rand;
	...
	for( int i=0; i<Nevts; i++ ){
		...
		else com = hClx->GetRandom() * TMath::DegToRad();
        depth = rand.Rndm(i) * thick;
        Eb_real = Eb + rand.Gaus( 0, dEb );
		Eb_real *= Ab;
		
		// calculate angle
		b_lab = projLab( com, Ab, At, Eb_real, Ex );
		t_lab = targLab( com, Ab, At, Eb_real, Ex );

		// calculate energy
        b_en = GetBEn( Ab, At, Eb_real, Ex, b_lab, com, thick, depth );
		t_en = GetTEn( Ab, At, Eb_real, Ex, t_lab, com, thick, depth );
 
		// detector resolution %
		b_en += rand.Gaus( 0, res*b_en*0.01 );
        t_en += rand.Gaus( 0, res*t_en*0.01 );
		
		// beam spot size affects position - calculate effective angle
		__xpos = rand.Gaus( 0, beam_sigma );
		b_xpos = cd_dist * TMath::ATan( b_lab ) + __xpos;
		t_xpos = cd_dist * TMath::ATan( t_lab ) + __xpos;
		b_lab = TMath::Tan( b_xpos / cd_dist );
		t_lab = TMath::Tan( t_xpos / cd_dist );

		// need to calculate lateral offset in terms of angle
		
		lab_lab->Fill( b_lab * TMath::RadToDeg(), t_lab * TMath::RadToDeg() );
        kin_lab_b->Fill( b_lab * TMath::RadToDeg(), b_en );
        kin_lab_t->Fill( t_lab * TMath::RadToDeg(), t_en );
        cd_sim->Fill( b_lab * TMath::RadToDeg(), b_en );
        cd_sim->Fill( t_lab * TMath::RadToDeg(), t_en );
		kin_com_b->Fill( com * TMath::RadToDeg(), b_en );
		kin_com_t->Fill( com * TMath::RadToDeg(), t_en );
